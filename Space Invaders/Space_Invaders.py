...
# Class dimensions of the projectile
class projectile(object):
	...
	# Function which draws the projectile
	def draw(self,screen):
...
while run:
	...
	# Bullet from ship hits the enemy
	for bullet in bullets:
		if bullet.y - bullet.radius	< stormtrooper.hitbox[1] + stormtrooper.hitbox[3] and bullet.y + bullet.radius > stormtrooper.hitbox[1]:
			if bullet.x + bullet.radius > stormtrooper.hitbox[0] and bullet.x - bullet.radius < stormtrooper.hitbox[0] + stormtrooper.hitbox[2]:
				stormtrooper.hit()
				bullets.pop(bullets.index(bullet))
	
	# Bullet speed from the ship
	for bullet in bullets:
		if bullet.y > 0:
	...
	# Bullet from enemy hits the ship
	for bullet in enemyBullets:
		if bullet.y - bullet.radius	< shipPos.hitbox[1] + shipPos.hitbox[3] and bullet.y + bullet.radius > shipPos.hitbox[1]:
			if bullet.x + bullet.radius > shipPos.hitbox[0] and bullet.x - bullet.radius < shipPos.hitbox[0] + shipPos.hitbox[2]:
				shipPos.hit()
				enemyBullets.pop(enemyBullets.index(bullet))
	
	# Bullet speed from the enemy
	for bullet in enemyBullets:
		if bullet.y < 600:
			bullet.y += bullet.speed
		else:
	...
	if key[pygame.K_SPACE]:
		if len(bullets) < 1:
			bullets.append(projectile(round(shipPos.x + shipPos.width //2), round(shipPos.y + shipPos.height //2), 6, (255,0,0), shipPos.speed))
	...
	if len(enemyBullets) < 1:
		...
		if seconds > 1:
			enemyBullets.append(projectile(round(stormtrooper.x + stormtrooper.width //2), round(stormtrooper.y + stormtrooper.height //2), 6, (0,255,0), stormtrooper.speed))
